...
INA219_Conf_Strc configuracc;
void INA219_INIT_Calibrate_CC(I2C_HandleTypeDef *hi2c,uint8_t dv_addr, float Rshunt, uint8_t MxpC){
	/* @brief : Set initial config and insert calibration parameter
	 * @param : hi2c - HAL_I2C used to read
	 * @param : dv_addr - address of INA219 device in the busline
	 * @param : Rshunt - R shunt value in ohm
	 * @param : MxpC - Max Expect Current in Ampere
	 * */

	////// -------------------- Configuration -------------------------------------
	configuracc.INA219CF.reset = 1;
	configuracc.INA219CF.BRNG = BRNG_FSR_32V;
	configuracc.INA219CF.PGA = PGA_GainD4_160mv;
	configuracc.INA219CF.BADC = ADCI_12bit_532uS;
	configuracc.INA219CF.SADC = ADCI_12bit_532uS;
	configuracc.INA219CF.Mode = INAM_ShuntBusV_Continuous;

	uint8_t confictor_si2c[2] = {configuracc.D8[1], configuracc.D8[0]};
	HAL_I2C_Mem_Write(hi2c, dv_addr, INA219_RG_Config, I2C_MEMADD_SIZE_8BIT, &confictor_si2c[0], 2, 10);
	...
	//// custom current LSB
	current_LSB = MxpC / 32768.0; // 2^15
	//calibrator.U16 = (int16_t)(trunc( 0.04096 / (current_LSB * INA219_R_SHUNT_Val))) << 1;
	calibrator.U16 = trunc( 0.04096 / (current_LSB * Rshunt));
#ifdef calibrate_EQ6
	//calibrator.U16 = trunc((calibrator.U16 * MeaShuntCurrent_ExtMeter) / INA219_Current_Raw);
#endif
	uint8_t calibrator_si2c[2] = {calibrator.U8[1], calibrator.U8[0]}; //// switch byte to send high order first in I2C
